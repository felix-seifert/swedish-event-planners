package com.felixseifert.swedisheventplanners.ui.views.staffrequest;
...
@Route(value = "staff-request", layout = MainView.class)
@PageTitle("Staff Requests | Swedish Event Planners")
@Secured({Role.ForAnnotation.HR_EMPLOYEE_WITH_PREFIX})
public class HRTeamView extends Div {
    ...
    private Button recruitButton = new Button();
    private Button outsourceButton = new Button();

    public HRTeamView(ProposalService proposalService) {
        ...
        recruitButton.addClickListener(this::recruitListener);

        outsourceButton.addClickListener(this::outsourceListener);
    ...
    private void recruitListener(ClickEvent<Button> e) {
        Proposal proposal = binder.getBean();
        if(proposal.getId() == null) {
            Notification.show("An exception happened while trying to handle the proposal.");
            return;
        }
        if(ProposalStatus.EXTRA_STAFF_REQUESTED.equals(proposal.getProductionProposalStatus())) {
            proposal.setProductionProposalStatus(ProposalStatus.STAFF_REQUEST_SOLVED_WITH_RECRUITMENT);
        }
        if(ProposalStatus.EXTRA_STAFF_REQUESTED.equals(proposal.getServiceProposalStatus())) {
            proposal.setServiceProposalStatus(ProposalStatus.STAFF_REQUEST_SOLVED_WITH_RECRUITMENT);
        }
        proposalService.putProposal(proposal);
        clearForm();
        refreshGrid();
        Notification.show(String.format("Staff request for %s handled with recruitment.", proposal.getRecordNumber()));
    }

    private void outsourceListener(ClickEvent<Button> e) {
        Proposal proposal = binder.getBean();
        if(proposal.getId() == null) {
            Notification.show("An exception happened while trying to handle the proposal.");
            return;
        ...
        if(ProposalStatus.EXTRA_STAFF_REQUESTED.equals(proposal.getProductionProposalStatus())) {
            proposal.setProductionProposalStatus(ProposalStatus.STAFF_REQUEST_SOLVED_WITH_OUTSOURCING);
        }
        if(ProposalStatus.EXTRA_STAFF_REQUESTED.equals(proposal.getServiceProposalStatus())) {
            proposal.setServiceProposalStatus(ProposalStatus.STAFF_REQUEST_SOLVED_WITH_OUTSOURCING);
        }
        proposalService.putProposal(proposal);
        clearForm();
        refreshGrid();
        Notification.show(String.format("Staff request for %s handled with outsourcing.", proposal.getRecordNumber()));
    }
    ...
    private FormLayout createFormLayout() {
        ...
        recruitButton.setText("Staff request solved with recruitment");
        recruitButton.addThemeVariants(ButtonVariant.LUMO_PRIMARY);
        outsourceButton.setText("Staff request solved with outsourcing");
        outsourceButton.addThemeVariants(ButtonVariant.LUMO_PRIMARY);
        ...
        return new FormLayout(recordNumberTextField, clientNameTextField, eventTypeTextField, new Label(),
                ...
                recruitButton, outsourceButton);
    }

    private void enableButtons(boolean enableButtons) {
        recruitButton.setEnabled(enableButtons);
        outsourceButton.setEnabled(enableButtons);
    }
    ...
    private List<Proposal> getItems() {
        return Stream.concat(
                proposalService.getAllProposalsByProductionStatus(Set.of(ProposalStatus.EXTRA_STAFF_REQUESTED)).stream(),
                proposalService.getAllProposalsByServiceStatus(Set.of(ProposalStatus.EXTRA_STAFF_REQUESTED)).stream())
